...
return s=s.split(" "),s.pop(),s.join(r?",":" ")+(t%1).toNDec(e||3)}function getStep(t,i,e){return[t,i].getIncrement(e)}function quadraticSolver(t,i,e,r){var n=Math.sqrt(i,2)-4*t*e
...
if(str=str.map(function(i){return i===t?void 0:i}),-1!=str.indexOf(void 0)&&str.length>0){var e=[]
for(i=0;i<str.length;i++)void 0!==str[i]&&e.push(isType(str[i],"Number")?parseFloat(str[i]):str[i])
str=e}return str}for(i=0;i<t.length;i++)str=str.remove(t[i])},Array.prototype.debug=function(){for(Essence.say("Debugging the following array: "+this),i=0;i<this.length;i++)Essence.say(i+": "+this[i])},Array.prototype.getOccurences=function(){var t=rmDuplicates(this),e=[],r=mkArray(t.length,2,0)
...
j++}return e},Array.prototype.bruteForceSort=function(){for(i=0;i<this.length;i++){for(s=this[i],pos=i,j=i+1;j<=this.length;j++)s>this[j]&&(s=this[j],pos=j)
var t=this[i]
...
return t},Array.prototype.inv=function(){return this.dotProd(1/this.det(),this.getAdjoint())},String.prototype.remove=function(t){var e=this
if(!isArray(t)){var r=e.split(t).map(function(i){return i===t?void 0:i}).join("")
return-1!=r.indexOf(void 0)?e.remove(void 0):r}for(i in t)e=e.remove(i)},Array.prototype.mix=function(){var t=mixedRange(0,1,this.length-1),e=this,r=[]
for(i=0;i<this.length;i++)r[i]=e[t[i]]
return r},String.prototype.toNDigits=function(t){var i=this
...
return t.mean(2)},String.prototype.normal=function(){return this.toLowerCase().remove(" ")},Number.prototype.length=function(){if(-1!=(this+"").indexOf("."))return[parseInt((this+"").split(".")[0]).length(),parseInt((this+"").split(".")[1]).length()]
